...
| `0x004D` | `WAIT_KEY` | - | - | - | ❌ |
| `0x004E` | `RELOAD_SAVE` | - | - | - | ❌ |
| `0x0058` | `BRANCH_ITEM` | item | quantity | fail_jump | ✅ |
| `0x005D` | `BRANCH_PARTY_POISON` | poison | fail_jump | - | ✅ |
| `0x005E` | `BRANCH_ENEMY_POISON` | poison | fail_jump | - | ✅ |
| `0x0061` | `BRANCH_PARTY_POISONED`| fail_jump | - | - | ✅ |
| `0x0064` | `CHECK_ENEMY_HP` | threshold | jump | - | ✅ |
| `0x0068` | `BRANCH_TARGET_PARTY` | jump | - | - | ✅ |
| `0x0074` | `BRANCH_PARTY_INJURED` | jump | - | - | ✅ |
| `0x0079` | `BRANCH_PARTY_HAS` | player | jump | - | ✅ |
| `0x0081` | `BRANCH_OBJ_VIEW` | event | - | jump | ✅ |
| `0x0083` | `BRANCH_OBJ_DIST` | event | distance | jump | ✅ |
| `0x0085` | `WAIT` | delay | - | - | ✅ |
| `0x0086` | `BRANCH_EQUIP_QTY` | item | quantity | jump | ✅ |
| `0x0091` | `BRANCH_SOLO_ENEMY` | jump | - | - | ✅ |
| `0x0094` | `BRANCH_EVENT_STATE` | event | state | jump | ✅ |
| `0x0095` | `BRANCH_IN_SCENE` | scene | jump | - | ✅ |
| `0x009C` | `ENEMY_DIVISION` | count | jump | - | ✅ |
| `0x009D` | `BRANCH_REVIVE_FAIL` | solo | jump | - | ❌ |
| `0x009E` | `ENEMY_SUMMON` | object | quantity | fail_jump | ✅ |
| `0x00A0` | `EXIT_GAME` | - | - | - | ❌ |
| `0x00A1` | `UPDATE_PLAYER` | - | - | - | ❌ |
| `0x00A2` | `RANDOM_MULTI_JUMP` | branches | - | - | ✅ |
...
| `0x00A4` | `SCROLL_IMAGE` | field_id | - | speed | ❌ |
| `0x00A5` | `FADE_IMAGE` | field_id | - | speed | ❌ |
...
| `0x0028` | `POISON_ENEMY` | all | poison | - | ❌ |
| `0x0029` | `POISON_PARTY` | all | poison | - | ❌ |
| `0x002A` | `CURE_ENEMY` | all | poison | - | ❌ |
| `0x002B` | `CURE_PARTY` | all | poison | - | ❌ |
| `0x002C` | `CURE_PARTY_LEVEL` | all | max_level | - | ❌ |
| `0x002E` | `SET_ENEMY_STATUS` | status | turns | fail_jump | ❌ |
| `0x0033` | `POUCH_COLLECT` | fail_jump | - | - | ❌ |
| `0x0034` | `COLLECT_ENEMIES` | jump | - | - | ✅ |
| `0x0039` | `DRAIN_ENEMY_HP` | amount | - | - | ❌ |
| `0x003A` | `FLEE_BATTLE` | fail_jump | - | - | ✅ |
| `0x0041` | `MAGIC_INVALID` | magic_id | target | player | ❌ |
| `0x0042` | `PARTY_USE_MAGIC` | magic_id | target | player | ❌ |
| `0x0045` | `SET_BATTLE_MUSIC` | music | - | - | ❌ |
| `0x004A` | `SET_BATTLEFIELD` | field_id | - | - | ❌ |
| `0x004B` | `NULLIFY_EVENT` | - | - | - | ❌ |
...
| `0x008A` | `AUTO_BATTLE` | - | - | - | ❌ |
| `0x0090` | `SET_ENEMY_SCRIPT` | event | script | type | ✅ |
| `0x009F` | `ENEMY_TRANSFORM` | target_id | - | - | ❌ |

#### Objects & Scripts
| Hex | Name | a | b | c | Status |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `0x0024` | `SET_AUTOSCRIPT` | event | script | - | ✅ |
...
| `0x006D` | `SET_SCENE_SCRIPT` | scene | enter | exit | ✅ |
| `0x0077` | `STOP_MUSIC` | - | - | - | ❌ |
...
### Script Flow and Branching Opcode Notes
- **`0x0034 COLLECT_ENEMIES`**: Jumps if the global collection value (`wCollectValue`) is 0 or less.
- **`0x003A FLEE_BATTLE`**: Jumps to the failure address if the attempt to flee the battle fails (standard in boss fights).
- **`0x0058 BRANCH_ITEM`**: Jumps to `c` if the party has fewer than `b` items of type `a`.
- **`0x0064 CHECK_ENEMY_HP`**: Jumps to `b` if the target enemy's HP percentage is greater than `a`.
- **`0x0068 BRANCH_TARGET_PARTY`**: Jumps if the current target is a party member (used in battle scripts).
- **`0x0074 BRANCH_PARTY_INJURED`**: Jumps if at least one party member is not at full HP.
- **`0x0079 BRANCH_PARTY_HAS`**: Jumps if the specified player (`a`) is currently active in the party.
- **`0x0081 BRANCH_OBJ_VIEW`**: Jumps if the player is NOT facing the specified event object.
- **`0x0083 BRANCH_OBJ_DIST`**: Jumps if the distance between the player and event `a` is greater than `b`.
- **`0x0086 BRANCH_EQUIP_QTY`**: Jumps if the party has equipped fewer than `b` instances of item `a`.
- **`0x0091 BRANCH_SOLO_ENEMY`**: Jumps if there is more than one enemy of the same type remaining.
- **`0x0094 BRANCH_EVENT_STATE`**: Jumps to `c` if event `a` is in state `b`.
- **`0x0095 BRANCH_IN_SCENE`**: Jumps if the current scene ID matches `a`.
- **`0x00A2 RANDOM_MULTI_JUMP`**: Jumps to one of the next `a` instructions at random.
