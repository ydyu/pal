 ...
 */
export function parseEnemyTeam(chunk, groupId) {
    const offset = groupId * TEAM_SIZE;
    if (offset + TEAM_SIZE > chunk.length)
        return [];
    const ids = [];
 ...
 */
function parseEnemyRecord(reader, index) {
 ...
 */
export function parseEnemyStatsTable(source) {
 ...
 */
export function getEffectiveAttack(level, attackOffset) {
 ...
 */
export function getEffectiveDefense(level, defenseOffset) {
