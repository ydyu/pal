import { ByteReader, WordBitReader, readUint16LE, readUint32LE } from "../binary.js";
/**
 * Checks if a buffer contains a YJ_1 signature ("YJ_1" at offset 0).
 */
export function isYj1Compressed(source) {
 ...
 */
function readLoop(reader, header) {
 ...
 */
function readCount(reader, header) {
    const mode = reader.readBits(2);
    if (mode !== 0) {
        if (reader.readBits(1) !== 0) {
 ...
 */
export function decompressYj1(source) {
    ...
    const nodes = new Array(treeLength + 1);
    ...
    const treeFootprint = (((treeLength & 0x0f) !== 0 ? (treeLength >> 4) + 1 : treeLength >> 4) << 1);
