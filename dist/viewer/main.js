...
import { Blitter, MemorySurface, PaletteRenderer, ResourceManager, SceneEngine, collectScriptRewards, getSpriteBounds, parseScript, pixelToTile, toSigned16, } from "../index.js";
import { DEFAULT_VIEWPORT_HEIGHT, DEFAULT_VIEWPORT_WIDTH, centerViewportOnPoint, clampViewport, findFirstPortraitId, formatRewardLabel, getSceneBounds, resolveAnimatedObjectFrame, resolveInitialSceneNumber, } from "./viewer-helpers.js";
const RESOURCE_FILE_SPECS = [
...
const RESOURCE_FILE_KEYS = new Set(RESOURCE_FILE_SPECS.map((spec) => spec.key));
const RESOURCE_FILE_BY_NAME = new Map(RESOURCE_FILE_SPECS.map((spec) => [spec.label.toUpperCase(), spec]));
...
const dom = {
    filesTabButton: requireElement("tab-files"),
    mapTabButton: requireElement("tab-map-view"),
    initializeButton: requireElement("initialize-button"),
    initStatus: requireElement("init-status"),
    libraryStatus: requireElement("library-status"),
    directoryInput: requireElement("pal-directory"),
    saveInput: requireElement("rpg-save"),
    sceneSelect: requireElement("scene-select"),
    centerPartyButton: requireElement("center-party-button"),
    viewerStatus: requireElement("viewer-status"),
    canvasShell: requireElement("viewer-canvas-shell"),
    canvas: requireElement("view-canvas"),
    placeholder: requireElement("viewer-placeholder"),
    inspectorSubtitle: requireElement("inspector-subtitle"),
    inspectorDetails: requireElement("inspector-details"),
    objectList: requireElement("scene-object-list"),
    objectListStatus: requireElement("object-list-status"),
    mobileTabs: requireElement("mobile-tabs"),
    mobileTabObjects: requireElement("mobile-tab-objects"),
    mobileTabInspector: requireElement("mobile-tab-inspector"),
    explorerSidebar: requireElement("explorer-sidebar"),
    inspectorSidebar: requireElement("inspector-sidebar"),
};
...
function formatMapNumber(mapNumber) {
...
function initFileInputs() {
    for (const inputId of ["pal-directory", "rpg-save"]) {
        const status = document.querySelector(`[data-file-status="${inputId}"]`);
        if (!status) {
...
async function handleDirectoryChange(input) {
    ...
    try {
        ...
        const loadedEntries = await Promise.all(Array.from(matchedFiles.entries()).map(async ([key, file]) => {
        ...
        if (missing.length > 0) {
            setStatus(status, `${prefix}: loaded ${loadedEntries.length} PAL files; missing ${missing.join(", ")}.`, "warning");
        }
        else {
            setStatus(status, `${prefix}: loaded ${loadedEntries.length} PAL files${foundLabels ? ` (${foundLabels})` : ""}.`, "success");
        }
        onFileSelectionChanged();
    }
    catch (error) {
...
async function handleSaveFileChange(input) {
    const inputId = input.id;
    const token = (fileReadTokens.get(inputId) ?? 0) + 1;
    fileReadTokens.set(inputId, token);
    const status = fileStatusElements.get(inputId);
...
function getCachedSprite(type, id) {
...
function getPartySpriteMap(saveData) {
    const map = new Map();
    if (!saveData) {
        return map;
    }
...
function buildRenderList(model, animationStep) {
    const sprites = [];
    ...
    for (const object of model.objects.values()) {
        ...
        sprites.push({
            directory,
            frameIndex: resolveAnimatedObjectFrame(object.direction, object.currentFrame, object.data.nSpriteFrames, animationStep, object.data.nSpriteFramesAuto),
            x: object.x,
            y: object.y,
...
function buildObjectSummaries(model, assets) {
...
function getScriptDetails(object) {
...
function describeSceneSource(sceneNumber) {
...
function updateViewerStatus(tone = "neutral") {
    if (!state.runtime) {
    ...
    setStatus(dom.viewerStatus, `Scene ${formatSceneNumber(sceneNumber)} / Map ${formatMapNumber(mapNumber)} · ${objectCount} objects${partySuffix}`, tone);
}
function populateSceneSelect(sceneCount, preferredScene) {
...
function getInitialViewport(sceneNumber, _model, bounds) {
    if (state.saveData && state.saveData.numScene === sceneNumber) {
        return {
...
function getCurrentSaveViewport() {
...
function loadScene(sceneNumber, recenter) {
    if (!state.assets) {
        throw new Error("Viewer assets are not initialized.");
    }
...
function buildInfoSection(title) {
...
function buildBadgeRow(labels, hidden) {
    const row = document.createElement("div");
...
function findSceneForSlot(slot) {
    if (!state.assets)
        return undefined;
...
function syncCurrentStateToHistory(fromAddr) {
...
function navigateScene(scene, slot, pushHistory = true, fromAddr) {
    if (!state.assets)
        return;
    if (pushHistory)
...
function buildParamElement(p, instIndex) {
    ...
    if (isScript) {
        el.type = "button";
        el.dataset.scriptIndex = String(p.raw);
        ...
        el.addEventListener("click", () => {
            ...
            if (!state.scriptStack.includes(targetIdx)) {
                ...
                const scrollContainer = dom.inspectorDetails.parentElement;
...
function buildScriptSection(title, scriptIndex, instructions, isCollapsed, onToggleCollapse, onClose, onPin, contextSpriteNum) {
...
function renderPortraitPreview(parent, portraitId) {
...
function renderTinyPreview(type, id, contextSpriteNum, dir) {
    if (!state.runtime || !state.assets)
        return null;
    if (type === "portrait" && (id === 0 || !state.files["RGM.MKF"]))
        return null;
    if (type === "worldFrame" && contextSpriteNum === undefined)
        return null;
    if (type === "item" && !state.files["BALL.MKF"])
        return null;
    if (type === "playerBattle" && !state.files["F.MKF"])
        return null;
    if (type === "enemyBattle" && !state.files["ABC.MKF"])
        return null;
    try {
        let directory;
        let frameIndex;
        ...
        canvas.getContext("2d").putImageData(new ImageData(new Uint8ClampedArray(rgba), frame.width, frame.height), 0, 0);
        return canvas;
    }
    catch {
        return null;
...
function buildSemanticGutter(inst, contextSpriteNum) {
...
function scrollScriptBlockIntoView(scriptIndex) {
    const blocks = Array.from(dom.inspectorDetails.querySelectorAll(`section[data-script-index="${scriptIndex}"]`));
    blocks[blocks.length - 1]?.scrollIntoView({ behavior: "smooth", block: "start" });
}
function scrollInstructionIntoView(addr) {
    const el = dom.inspectorDetails.querySelector(`[data-addr="${addr}"]`);
    el?.scrollIntoView({ behavior: "smooth", block: "center" });
}
function extractScriptPreview(instructions) {
    for (const inst of instructions) {
        for (const p of inst.params) {
            if (p.type === "dialogue") {
...
function appendScriptStack() {
    ...
    for (const stackIdx of state.scriptStack) {
        ...
        dom.inspectorDetails.append(buildScriptSection(`Script ${formatHex(stackIdx)}`, stackIdx, stackInstructions, isCollapsed, () => {
            const scrollContainer = dom.inspectorDetails.parentElement;
            const savedScrollTop = scrollContainer.scrollTop;
...
function renderInspector() {
    ...
    if (!selected) {
        ...
        sceneSection.append(buildInfoGrid([
        ...
        const enterInstructions = state.assets
            ...
            : [];
        dom.inspectorDetails.append(buildScriptSection("Script on Enter", enterIdx, enterInstructions, false, undefined, undefined, makePinCallback(enterIdx)));
        ...
        if (teleportIdx !== 0 && teleportIdx !== enterIdx) {
            const teleportInstructions = state.assets
                ...
                : [];
            dom.inspectorDetails.append(buildScriptSection("Script on Teleport", teleportIdx, teleportInstructions, false, undefined, undefined, makePinCallback(teleportIdx)));
        }
        appendScriptStack();
        return;
    }
    ...
    objectSection.append(buildInfoGrid([
    ...
    dom.inspectorDetails.append(buildScriptSection("Trigger Script", selected.data.triggerScript, details.triggerInstructions, false, undefined, undefined, () => {
    ...
    if (selected.data.autoScript !== 0 && selected.data.autoScript !== selected.data.triggerScript) {
        dom.inspectorDetails.append(buildScriptSection("Auto Script", selected.data.autoScript, details.autoInstructions, false, undefined, undefined, () => {
...
function getObjectSpriteHit(object, worldX, worldY) {
    if (object.state === 0 || object.data.spriteNum <= 0) {
        return null;
    }
    const directory = getCachedSprite("world", object.data.spriteNum);
    const frameIndex = resolveAnimatedObjectFrame(object.direction, object.currentFrame, object.data.nSpriteFrames, state.animationStep, object.data.nSpriteFramesAuto);
    const frame = blitter.getFrame(directory, frameIndex, `world:${object.data.spriteNum}`);
    if (!frame) {
        return null;
    }
    const bounds = getSpriteBounds(object.x, object.y, object.data.sLayer, frame.width, frame.height, object.data.sLayer * 8 + 9, object.data.sLayer * 8 + 2);
...
function getAnchorProximityScore(object, worldX, worldY) {
    if (object.state === 0) {
        return null;
    }
...
function pickObjectAt(worldX, worldY) {
    ...
    let bestSpriteHit = null;
    let bestAnchorHit = null;
...
function selectObject(slot, centerOnObject) {
    if (!state.runtime) {
        return;
    }
    ...
    if (slot !== undefined) {
        ...
        if (object && centerOnObject) {
            ...
            state.viewport = centerViewportOnPoint(object.x, object.y, canvasCenterX, canvasCenterY, state.viewport.width, state.viewport.height, state.runtime.bounds);
            invalidateRender();
        }
    }
    renderInspector();
...
function drawOverlayLabel(ctx, x, y, text) {
...
function initControls() {
    ...
    window.addEventListener("popstate", (event) => {
        if (!state.initialized || !state.assets)
            return;
        const s = event.state;
        if (!s?.scene)
            return;
